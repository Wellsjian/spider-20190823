...
获取root权限   sudo -i   tarena
查看文件尾部20行   tail -20 /var/log/redis/redis-server.log
查看文件    vi /var/log/redis/redis-server.log
```
...
1、客户端向服务器发送BGREWRITEAOF命令
   ...
   Background append only file rew riting started
...
3、protected-mode yes|no   属性改为No
# rdb持久化-默认配置
4、dir /var/lib/redis
...
```Python
1.配置文件： sudo /etc/redis/redic.conf
2.备份文件： sudo /var/lib/redis/
3.日志文件： sudo /var/log/redis/redis-server.log
4.启动文件： sudo /etc/init.d/redis-server
```
...
2.创建配置文件  sentinel.conf
	port 26379
	sentinel monitor 名字 host port quorum
	sentinel monitor tedu 127.0.0.1 6379
3.启动sentinel服务
